...
function get_preserve_id()
    global preserve_id += 1
    return preserve_id
end

persistent_id = -1

function get_persistent_id()
    global persistent_id += 1
...
"""
    ...
    it converts @preserve annotated expr into Expr(:preserve, ...)
"""
function convert_ast_with_hole(expr::Expr)
    if expr.head == :macrocall
        ...
        elseif annot isa Symbol && string(annot) == "@preserve"
            # @preserve should be used on assignments: @preserve x = expr
            # Convert to Expr(:preserve, assignment_expr, preserve_id)
            newargs = collect(
                filter(
                    x -> !(x isa LineNumberNode),
                    args[2:end]
                ))
            return Expr(:preserve, newargs..., get_preserve_id())
        elseif annot isa Symbol && string(annot) == "@persistent"
            # @persistent should be used on assignments: @persistent x = expr
            # Convert to Expr(:persistent, assignment_expr, persistent_id)
            newargs = collect(
                filter(
                    x -> !(x isa LineNumberNode),
                    args[2:end]
                ))
            return Expr(:persistent, newargs..., get_persistent_id())
        end
        return expr
    elseif expr.head == :if
