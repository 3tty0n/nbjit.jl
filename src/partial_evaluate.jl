...
function partial_evaluate(expr, unfolded_vars, env)
    ...
    if head == :macrocall
        ...
        if string(annot) == "@hole"
            return
        elseif string(annot) == "@preserve"
            # Handle @preserve macrocall (before conversion to :preserve AST)
            # Extract the assignment and evaluate the RHS
            for i in 2:length(args)
                if args[i] isa Expr && args[i].head == :(=)
                    lhs = args[i].args[1]
                    rhs = args[i].args[2]
                    # Evaluate the RHS using Julia's eval
                    local rhs_val
                    try
                        rhs_val = Base.eval(Main, rhs)
                        env[lhs] = rhs_val
                    catch e
                        @warn "Failed to evaluate @preserve RHS: $e"
                        rhs_val = rhs
                        env[lhs] = rhs
                    end
                    # Emit the assignment with the evaluated constant value
                    # so that LLVM codegen can define the variable
                    return Expr(:(=), lhs, rhs_val)
                end
            end
            return nothing
        elseif string(annot) == "@persistent"
            # Handle @persistent macrocall (before conversion to :persistent AST)
            # Extract the assignment and evaluate the RHS once, storing as a constant
            for i in 2:length(args)
                if args[i] isa Expr && args[i].head == :(=)
                    lhs = args[i].args[1]
                    rhs = args[i].args[2]
                    ...
                    try
                        ...
                        @warn "Failed to evaluate @persistent RHS: $e"
                        rhs_val = rhs
                        env[lhs] = rhs
                    end
                    # Emit the assignment with the evaluated constant value
    ...
    elseif head == :preserve
        # Handle :preserve AST node (after conversion by convert_ast_with_hole)
        # The args contain the assignment expression and the preserve_id
        # Find the assignment in args (skip the preserve_id at the end)
        for i in 1:length(expr.args)-1
            arg = expr.args[i]
            if arg isa Expr && arg.head == :(=)
                lhs = arg.args[1]
                rhs = arg.args[2]
                # First try to get value from env if RHS symbols are already evaluated
                rhs_eval = partial_evaluate(rhs, unfolded_vars, env)
                local final_val
                if is_constant(rhs_eval)
                    ...
                    # Try to evaluate using Julia's eval
                    try
                        rhs_val = Base.eval(Main, rhs)
                        env[lhs] = rhs_val
                        final_val = rhs_val
                ...
                # Emit the assignment with the evaluated constant value
                # so that LLVM codegen can define the variable
                return Expr(:(=), lhs, final_val)
            end
        end
        return nothing
    elseif head == :persistent
        # Handle :persistent AST node (after conversion by convert_ast_with_hole)
        # The args contain the assignment expression and the persistent_id
        # Find the assignment in args (skip the persistent_id at the end)
        for i in 1:length(expr.args)-1
            arg = expr.args[i]
            if arg isa Expr && arg.head == :(=)
                lhs = arg.args[1]
    ...
    elseif head == :(=) || head in (:+=, :-=, :*=, :/=, :^=, :%=)
        ...
        lhs_sym = lhs
        # Add to env for variables that CAN be folded (not in unfolded_vars)
        # Don't add unfolded variables to env as they should remain symbolic
        # Don't propagate array/object expressions (they need to remain as variables for codegen)
        if op_head == :(=)
            is_object_expr = rhs_val isa Expr && (
                rhs_val.head == :vect ||
                (rhs_val.head == :call && length(rhs_val.args) >= 1 &&
                 rhs_val.args[1] in (:zeros, :ones, :fill, :Vector, :Dict))
            )
            if !(lhs_sym in unfolded_vars) && !is_object_expr
                env[lhs_sym] = rhs_val
            end
        else
            base_op = Symbol(String(op_head)[1:end-1])
            if !(lhs_sym in unfolded_vars)
                current_val = get(env, lhs_sym, nothing)
                # Only fold if both current value and rhs are constants
                if current_val !== nothing && is_constant(current_val) && is_constant(rhs_val)
                    op_func = getfield(Base, base_op)
                    env[lhs_sym] = op_func(current_val, rhs_val)
                end
                # If not both constants, don't update env (keep it symbolic)
            end
            # For compound assignment, generate the full expression
            old_val = Expr(:call, base_op, lhs_sym, rhs_val)
            return Expr(:(=), lhs_sym, old_val)
        end

        # Always return assignment expressions - don't eliminate them
        # (They may have side effects or be needed for mutable state)
        return Expr(:(=), lhs_sym, rhs_val)
    elseif expr.head == :tuple || expr.head == :vect
        elems = [partial_evaluate(a, unfolded_vars, env) for a in expr.args]
        if expr.head == :tuple && all(is_constant, elems)
            return tuple(elems...)
        end
        # Keep :vect as Expr so codegen can handle array creation
    ...
    elseif head == :for
        ...
        if iter_spec isa Expr && iter_spec.head == :(=)
                        ...
                        if is_constant(start_val) && is_constant(end_val)
                            ...
                            if length(range_vals) <= 10
                                ...
                                for val in range_vals
                                    # Only add loop variable to environment, not other variables
                                    # This prevents over-folding of mutable variables in loop body
                                    loop_env = Dict{Symbol, Any}(iter_var => val)
            ...
            # Don't pass environment to avoid folding mutable loop variables
            new_body = partial_evaluate(body, unfolded_vars, Dict{Symbol, Any}())
...
const _FUNC_NAMESPACE = string(time_ns())

function fresh_func_name()
    _FUNC_COUNTER[] += 1
    return Symbol("func_" * string(_FUNC_COUNTER[]) * "_" * _FUNC_NAMESPACE)
end
...
function partial_evaluate_and_make_entry(code; params::Vector{Symbol}=Symbol[], fname::Union{Nothing,Symbol}=nothing)
    ...
    if last_expr !== nothing
        ...
        if last_expr isa Symbol
            # Always return the symbol itself, not its folded value from env
            # The variable may have been modified, so we can't use the cached value
            return_val = last_expr
            if !(last_expr in unfolded_vars)
                # For non-unfolded vars, use the symbol
            end
