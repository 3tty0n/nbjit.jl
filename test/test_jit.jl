...
@testset "Enriched JIT Features" begin
    ...
    @testset "Float64 Support" begin
        @testset "Float64 arithmetic" begin
            ...
            @test result isa Float64  # Result preserves Float64
            @test result ≈ 5.85
        end
...
@testset "For Loop Tests" begin
    session = IJuliaIntegration.NotebookSession()

    @testset "Simple for loop with accumulator" begin
        code = quote
            result = 0
            for i in 1:10
                result = result + i
        ...
        result = IJuliaIntegration.run_pure_cell!(session, code, "loop_sum")
        @test result == 55  # Sum of 1 to 10
    end

    @testset "For loop with multiplication" begin
        code = quote
            result = 1
            for i in 1:5
                result = result * i
        ...
        result = IJuliaIntegration.run_pure_cell!(session, code, "loop_product")
        @test result == 120  # 5!
    end

    @testset "For loop modifying variable" begin
        code = quote
            x = 0
            for i in 1:3
                x = i
        ...
        result = IJuliaIntegration.run_pure_cell!(session, code, "loop_assign")
        @test result == 3  # Last value of i
    end
end
